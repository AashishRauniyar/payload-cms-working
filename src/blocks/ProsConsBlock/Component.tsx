...
export function ProsConsBlock({
  ...
  const defaultPros = [
    { point: "Exceptional Audio Quality - Crystal clear highs, rich mids, and deep bass that rivals $400+ headphones" },
    { point: "Incredible Comfort - Wore for 8+ hours without discomfort during work sessions" },
    { point: "Outstanding Battery Life - Actually delivers 40+ hours with ANC on - exceeded expectations" }
  ];

  const defaultCons = [
    { point: "Slightly Bulky for Travel - Larger than ultra-portable options, but case helps with storage" },
    { point: "App Could Be Better - Mobile app works but interface feels dated compared to competitors" },
  ...
  const { pros: parsedPros, cons: parsedCons } = tableData ? parseProsCons(tableData) : { pros: [], cons: [] };
    ...
    <div className="grid grid-cols-2 gap-8">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="text-xl font-bold text-green-600">{prosTitle}</div>
        </div>
          ...
          {prosData.map((item, index) => {
            ...
            return (
              <div key={index} className="flex gap-3 items-center">
                ...
                <div>
                  <div className="font-semibold text-gray-900 mb-1">{formatted.title}</div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="text-xl font-bold text-orange-600">{consTitle}</div>
        </div>
        ...
        <div className="space-y-6">
          {consData.map((item, index) => {
            const formatted = formatPoint(item.point);
            return (
              <div key={index} className="flex gap-3 flex items-center">
                <div className="flex-shrink-0 mt-1 flex items-center justify-center mb-2">
    ...
    return (
      <section className={`py-4 px-0 ${backgroundClasses[backgroundColor]} ${className}`}>
        <div className="text-center mb-8">
          <p className="text-gray-600 text-sm mb-2">Based on 30 days of real-world testing across different scenarios</p>
        </div>
        {title && <h2 className="text-3xl font-bold text-center mb-4 text-gray-800">{title}</h2>}
        {content}
      </section>
  ...
  return (
    <section className={`py-6 px-0 ${backgroundClasses[backgroundColor]} min-h-screen ${className}`}>
      <div className="max-w-6xl mx-auto p-6">
        <div className="text-center mb-8">
          <p className="text-gray-600 text-sm mb-2">Based on 30 days of real-world testing across different scenarios</p>
        </div>
        {title && <div className="text-3xl font-bold text-center mb-8 text-gray-800">{title}</div>}
        {content}
      </div>
    </section>
  );
