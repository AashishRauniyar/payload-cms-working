...
const StarRating: React.FC<{ rating: number }> = ({ rating }) => {
  ...
  return (
    <div className="flex items-center gap-1 justify-center">
      ...
      <span className="ml-2 text-blue-600 font-medium text-sm">{rating}</span>
    </div>
  )
}
    ...
    <div className="max-w-7xl mx-auto p-8">
      ...
      <div className="border border-gray-200 bg-white">
        ...
        <div className="bg-blue-600 text-gray-900">
          <div className="flex">
            <div className="w-1/4 p-5 font-bold text-lg border-r border-blue-700">
              What To Look For
            </div>
            {productList.map((p, i) => {
              const rating = Number.isFinite(p?.rating)
              ...
              return (
                <div
                  className="w-1/4 p-5 text-center border-r border-blue-700 last:border-r-0"
                  key={`rating-${i}`}
                >
                  <div className="font-bold mb-3">{p.name}</div>
                  <StarRating rating={rating} />
                </div>
              )
            })}
